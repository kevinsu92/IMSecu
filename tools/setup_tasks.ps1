...
#
# 14:50 한 번만 보면 문제를 집행 10분 전에 알게 되는데, 사용자가 학교에 있으면
# 그때는 고칠 수가 없다. 등교 전(08:40)과 점심시간(12:30)에도 확인해 두면
# 쉬는 시간에 대응할 여지가 생긴다. 점검은 클릭을 하지 않는 읽기 전용이라
# 장중에 돌아도 매매에 영향이 없다.
...
#   (2) 14:50 에 실패를 알아도 사용자는 수업 중이라 15:05 까지 15분뿐이다.
$hcTriggers = @()
